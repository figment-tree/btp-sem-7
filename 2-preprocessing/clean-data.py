...
def check_data_integrity_del():
    ...
    for i in range(data.shape[0]):
        ...
        loc1 = data[i, 1]
        loc2 = data[i, 2]
        filename = str(int(year)) + '_' + str(int(loc1)) + \
            '_' + str(int(loc2)) + '.npy'
...
def merge_image(MODIS_img_list, MODIS_temperature_img_list):
    ...
    for i in range(0, len(MODIS_img_list)):
        ...
        img_shape_new = (img_shape[0], img_shape[1],
                         img_shape[2] + img_temperature_shape[2])
        merge = np.empty(img_shape_new)
        for j in range(0, img_shape[2] / 7):
            img = MODIS_img_list[i][:, :, (j * 7):(j * 7 + 7)]
            temperature = MODIS_temperature_img_list[i][:,
                                                        :, (j * 2):(j * 2 + 2)]
            merge[:, :, (j * 9):(j * 9 + 9)
                  ] = np.concatenate((img, temperature), axis=2)
...
#                 #     continue
#                 MODIS_temperature_path = os.path.join(
#                     MODIS_temperature_dir, file)
...
#                 # read image
#                 try:
#                     MODIS_img = np.transpose(
#                         np.array(gdal.Open(MODIS_path).ReadAsArray(), dtype='uint16'), axes=(1, 2, 0))
#                 except ValueError as msg:
#                     print msg
#                     continue
#                 # read temperature
#                 MODIS_temperature_img = np.transpose(np.array(gdal.Open(
#                     MODIS_temperature_path).ReadAsArray(), dtype='uint16'), axes=(1, 2, 0))
#                 # shift
...
#                 # read mask
#                 MODIS_mask_img = np.transpose(np.array(
#                     gdal.Open(MODIS_mask_path).ReadAsArray(), dtype='uint16'), axes=(1, 2, 0))
#                 # Non-crop = 0, crop = 1
...
#                 MODIS_temperature_img_list = divide_image(
#                     MODIS_temperature_img, 0, 46 * 2, 14)
...
#                 MODIS_list = merge_image(
#                     MODIS_img_list, MODIS_temperature_img_list)
#
#                 # Do the mask job
...
#                         # save as .npy
#                         filename = img_output_dir + \
#                             str(year) + '_' + str(loc1) + \
#                             '_' + str(loc2) + '.npy'
#                         np.save(filename, MODIS_list_masked[i])
#                         filename, ':written ', str(count)
#                         count += 1
...
def preprocess_save_data_parallel(file):
    ...
    if file.endswith(".tif"):
        ...
        MODIS_mask_img = np.transpose(np.array(
            gdal.Open(MODIS_mask_path).ReadAsArray(), dtype='uint16'), axes=(1, 2, 0))
        # Non-crop = 0, crop = 1
        # In LC_Type1, a value of 12 denotes croplands
        MODIS_mask_img[MODIS_mask_img != 12] = 0
        MODIS_mask_img[MODIS_mask_img == 12] = 1
        ...
        for i in range(0, 14):
            ...
            if np.sum(np.all(data_yield[:, 0:3] == key, axis=1)) > 0:
                ...
                filename = img_output_dir + \
                    str(year) + '_' + str(loc1) + '_' + str(loc2) + '.npy'
                np.save(filename, MODIS_list_masked[i])
...
if __name__ == "__main__":
    ...
    for _, _, files in os.walk(MODIS_dir):
        Parallel(n_jobs=12)(delayed(preprocess_save_data_parallel)(file)
                            for file in files)
